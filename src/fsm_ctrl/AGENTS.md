...
- **Landing completion**: Landing entry uses `ResetLanding`, matching the active `TickLanding/prepareLanding` path; it does not initialize the unused closed-loop planner. Neither landing Tick path requests OFFBOARD or arm. Height and vertical-speed conditions must remain valid for `landing_confirmation_seconds` before completion. Completion sets a process-lifetime permanent safety lock, publishes zero thrust while armed, and requests a normal disarm through `/mavros/cmd/arming`, retrying at `landing_disarm_retry_seconds`. Once telemetry reports disarmed, thrust publication stops. The permanent lock rejects every later state command and blocks all automatic arm requests, so only restarting the process can clear it. Landing vision observations are currently monitor-only and do not participate in the control loop.
