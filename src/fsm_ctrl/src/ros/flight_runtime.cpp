...
using MissionPoint = fsm_ctrl::ros_adapter::MissionPoint;
...
class RosMissionPort final : public flight::MissionPort {
 ...
 private:
  ...
  void loadSuperTrajectory() {
    ...
    try {
      super_segments_ = fsm_ctrl::ros_adapter::loadMissionSegments(waypoints_file_);
...
class FlightFsmNode {
 public:
  FlightFsmNode()
      : private_node_("~"),
        ...
        config_(fsm_ctrl::ros_adapter::loadConfig(private_node_)),
        context_(clock_, autopilot_, setpoint_, nmpc_, mission_, landing_, camera_control_,
                 config_),
        machine_(context_),
        dispatcher_(machine_, &mission_),
        mailbox_(fsm_ctrl::ros_adapter::loadUdpPort(private_node_)) {
 ...
 private:
  ...
  fsm_ctrl::ros_adapter::UdpCommandMailbox mailbox_;     // UDP 命令 mailbox。
...
int fsm_ctrl::ros_adapter::runFlightRuntime() {
  try {
    FlightFsmNode node;
    node.run();
    return 0;
