...
NmpcController::NmpcController(const std::array<double, 2>& vertical_acceleration_limits,
                               const std::array<double, 2>& angular_rate_limits,
                               int prediction_steps, double prediction_step_seconds, int state_size,
                               int input_size, const Eigen::Matrix<float, 3, 1>& position_weights,
  ...
  for (int j = 0; j < prediction_steps_; j++) initial_guess_.push_back(0);  // wx[0..N-1]
  for (int j = 0; j < prediction_steps_; j++) initial_guess_.push_back(0);  // wy[0..N-1]
  for (int j = 0; j < prediction_steps_; j++) initial_guess_.push_back(0);  // wz[0..N-1]
  for (int j = 0; j < prediction_steps_; j++) initial_guess_.push_back(0);  // az[0..N-1]
  ...
  casadi::SX x_dot = casadi::SX::vertcat({
      ...
      (q(0) * q(0) - q(1) * q(1) - q(2) * q(2) + q(3) * q(3)) * acc_z - kGravity,
  ...
  // 求解问题符号表示
  casadi::SX input_trajectory =
      casadi::SX::sym("input_trajectory", input_size_,
                      prediction_steps_);  // 控制输入，本案例中为 4*(predict_step)
  casadi::SX state_trajectory =
      casadi::SX::sym("state_trajectory", state_size_,
                      prediction_steps_ + 1);  // 状态输出，本案例中为 13*(predict_step+1)

  // 优化问题所需参数
  // 当前所有状态+期望轨迹点对应位置、速度、姿态、加速度+期望控制量
  // 10+(1+predict_step)*10+predict_step*4
  casadi::SX opt_param = casadi::SX::sym(
      "opt_param", (prediction_steps_ + 2) * state_size_ + prediction_steps_ * input_size_);
  ...
  // 根据模型函数前向预测无人机运动状态
  state_trajectory(casadi::Slice(), 0) = opt_param(casadi::Slice(0, state_size_, 1));  // 状态初始值
  ...
  for (int i = 0; i < prediction_steps_; i++) {
    ...
    casadi::SX current_input = casadi::SX::vertcat({opt_var(i), opt_var(i + prediction_steps_),
    ...
    state_trajectory(casadi::Slice(), i + 1) =
        model_function(model_inputs).at(0) * prediction_step_seconds_ + current_state;
  }

  // 惩罚矩阵
  casadi::SX position_cost = casadi::SX::diag(
      {casadi::SX::vertcat({position_weights_(0), position_weights_(1), position_weights_(2)})});
  casadi::SX velocity_cost = casadi::SX::diag(
      {casadi::SX::vertcat({velocity_weights_(0), velocity_weights_(1), velocity_weights_(2)})});
  casadi::SX attitude_cost = casadi::SX::diag(
      {casadi::SX::vertcat({attitude_weights_(0), attitude_weights_(1), attitude_weights_(2)})});
  casadi::SX angular_rate_cost = casadi::SX::diag({casadi::SX::vertcat(
      {angular_rate_weights_(0), angular_rate_weights_(1), angular_rate_weights_(2)})});
  ...
  for (int i = 0; i < prediction_steps_; i++) {
    casadi::SX error_pos =
        opt_param(casadi::Slice(state_size_ * (i + 1), state_size_ * (i + 1) + 3, 1)) -
        state_trajectory(casadi::Slice(0, 3, 1), i);
    casadi::SX error_vel =
        opt_param(casadi::Slice(state_size_ * (i + 1) + 3, state_size_ * (i + 1) + 6, 1)) -
        state_trajectory(casadi::Slice(3, 6, 1), i);
    casadi::SX error_quat =
        casadi::SX::vertcat({state_trajectory(6, i) * opt_param(state_size_ * (i + 1) + 7) -
    ...
    casadi::SX error_w =
        opt_param(casadi::Slice((state_size_ * (prediction_steps_ + 2) + input_size_ * i),
    ...
    cost_function = cost_function + casadi::SX::mtimes({error_pos.T(), position_cost, error_pos}) +
  ...
  casadi::SX error_pos = opt_param(casadi::Slice(state_size_ * (prediction_steps_ + 1),
                                                 state_size_ * (prediction_steps_ + 1) + 3, 1)) -
                         state_trajectory(casadi::Slice(0, 3, 1), prediction_steps_);
  casadi::SX error_vel = opt_param(casadi::Slice(state_size_ * (prediction_steps_ + 1) + 3,
                                                 state_size_ * (prediction_steps_ + 1) + 6, 1)) -
                         state_trajectory(casadi::Slice(3, 6, 1), prediction_steps_);
  casadi::SX error_quat =
      casadi::SX::vertcat({state_trajectory(6, prediction_steps_) *
...
void NmpcController::solve(const std::vector<double>& current_states,
  ...
  size_t desired_params_size =
      static_cast<size_t>((prediction_steps_ + 1) * state_size_ + prediction_steps_ * input_size_);
  // 向量大小检查
  ...
  if (solve_time_ms > 20.0) {
    ROS_ERROR("NMPC SOLVE TOOK %.1f ms (>20ms! OFFBOARD TIMEOUT RISK!) cost=%.4f", solve_time_ms,
              final_cost);
  } else if (solve_time_ms > 10.0) {
    ROS_WARN("NMPC solve time: %.1f ms (>10ms, loop budget tight) cost=%.4f", solve_time_ms,
             final_cost);
  } else {
  ...
  cmd_control_wx.assign(cmd_control_all.begin(), cmd_control_all.begin() + prediction_steps_);
  ...
  // 控制序列的第一组作为当前控制量
  angular_rate_command_ << cmd_control_wx.front(), cmd_control_wy.front(), cmd_control_wz.front();
