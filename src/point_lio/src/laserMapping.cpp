...
void publish_init_map(const ros::Publisher & pubLaserCloudFullRes)
{
    ...
    laserCloudmsg.header.frame_id = output_frame_id;
...
void publish_frame_body(const ros::Publisher & pubLaserCloudFull_body)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        const PointType &point_imu = laserCloudIMUBody->points[i];
        PointType &point_body = laserCloudBody->points[i];
        const V3D p_body = Body_R_wrt_IMU * V3D(point_imu.x, point_imu.y, point_imu.z)
                           + Body_T_wrt_IMU;
        point_body.x = p_body.x();
        point_body.y = p_body.y();
        point_body.z = p_body.z();
        point_body.intensity = point_imu.intensity;
    }

    sensor_msgs::PointCloud2 laserCloudmsg;
    pcl::toROSMsg(*laserCloudBody, laserCloudmsg);
    laserCloudmsg.header.stamp = ros::Time().fromSec(lidar_end_time);
    laserCloudmsg.header.frame_id = output_child_frame_id;
    pubLaserCloudFull_body.publish(laserCloudmsg);
    // publish_count -= PUBFRAME_PERIOD;
}

void get_body_state(V3D &position, M3D &rotation, V3D &velocity,
                    V3D &angular_velocity)
{
    V3D imu_position;
    M3D imu_rotation;
    V3D world_velocity;
    V3D imu_angular_velocity;
    if (!use_imu_as_input)
    {
        imu_position = kf_output.x_.pos;
        imu_rotation = kf_output.x_.rot;
        world_velocity = kf_output.x_.vel;
        imu_angular_velocity = kf_output.x_.omg;
    }
    else
    {
        imu_position = kf_input.x_.pos;
        imu_rotation = kf_input.x_.rot;
        world_velocity = kf_input.x_.vel;
        imu_angular_velocity = input_in.gyro - kf_input.x_.bg;
    }

    // Body_R_wrt_IMU rotates vectors from the MID360 IMU frame into the
    // aircraft body frame. Body_T_wrt_IMU is the IMU origin expressed in body.
    rotation = imu_rotation * Body_R_wrt_IMU.transpose();
    position = imu_position - rotation * Body_T_wrt_IMU;
    velocity = rotation.transpose() * world_velocity;
    angular_velocity = Body_R_wrt_IMU * imu_angular_velocity;
}

template<typename T>
void set_posestamp(T &out)
{
    V3D position, velocity, angular_velocity;
    M3D rotation;
    get_body_state(position, rotation, velocity, angular_velocity);
...
void set_covariance_block(CovarianceType &output, int output_row, int output_col,
                          const M3D &block)
{
    for (int row = 0; row < 3; ++row)
    {
        ...
        {
            output[(output_row + row) * 6 + output_col + col] = block(row, col);
        }
    }
}

void set_odometry_covariances(nav_msgs::Odometry &odom, const M3D &rotation)
{
    ...
    M3D position_covariance;
    M3D orientation_covariance;
    M3D velocity_covariance;
    M3D angular_velocity_covariance;
    if (!use_imu_as_input)
    {
        const Eigen::Matrix<double, 30, 30> &P = kf_output.get_P();
        position_covariance = P.block<3, 3>(0, 0);
        orientation_covariance = Body_R_wrt_IMU * P.block<3, 3>(3, 3)
                                 * Body_R_wrt_IMU.transpose();
        velocity_covariance = rotation.transpose() * P.block<3, 3>(12, 12) * rotation;
        angular_velocity_covariance = Body_R_wrt_IMU * P.block<3, 3>(15, 15)
                                      * Body_R_wrt_IMU.transpose();
    }
    else
    {
        const Eigen::Matrix<double, 24, 24> &P = kf_input.get_P();
        position_covariance = P.block<3, 3>(0, 0);
        orientation_covariance = Body_R_wrt_IMU * P.block<3, 3>(3, 3)
                                 * Body_R_wrt_IMU.transpose();
        velocity_covariance = rotation.transpose() * P.block<3, 3>(12, 12) * rotation;
        angular_velocity_covariance = M3D::Identity() * std::max(gyr_cov_input, 1e-6);
    }

    set_covariance_block(odom.pose.covariance, 0, 0, position_covariance);
    set_covariance_block(odom.pose.covariance, 3, 3, orientation_covariance);
    set_covariance_block(odom.twist.covariance, 0, 0, velocity_covariance);
    set_covariance_block(odom.twist.covariance, 3, 3, angular_velocity_covariance);
}

void publish_odometry(const ros::Publisher & pubOdomAftMapped)
{
    const double stamp_sec = publish_odometry_without_downsample ? time_current : lidar_end_time;
    static double last_publish_time = -std::numeric_limits<double>::infinity();
    if (!std::isfinite(stamp_sec)) return;
    if (stamp_sec < last_publish_time) last_publish_time = -std::numeric_limits<double>::infinity();
    const double min_period = 1.0 / odometry_publish_rate_hz;
    if (stamp_sec - last_publish_time + 1e-9 < min_period) return;
    last_publish_time = stamp_sec;

    odomAftMapped.header.frame_id = output_frame_id;
    odomAftMapped.child_frame_id = output_child_frame_id;
    odomAftMapped.header.stamp = ros::Time().fromSec(stamp_sec);
    set_posestamp(odomAftMapped.pose.pose);

    V3D position, velocity, angular_velocity;
    M3D rotation;
    get_body_state(position, rotation, velocity, angular_velocity);
    odomAftMapped.twist.twist.linear.x = velocity.x();
    odomAftMapped.twist.twist.linear.y = velocity.y();
    odomAftMapped.twist.twist.linear.z = velocity.z();
    odomAftMapped.twist.twist.angular.x = angular_velocity.x();
    odomAftMapped.twist.twist.angular.y = angular_velocity.y();
    odomAftMapped.twist.twist.angular.z = angular_velocity.z();
    set_odometry_covariances(odomAftMapped, rotation);

    pubOdomAftMapped.publish(odomAftMapped);

    if (!odometry_publish_tf) return;
...
void publish_path(const ros::Publisher pubPath)
{
    ...
    msg_body_pose.header.frame_id = output_frame_id;
    static int jjj = 0;
    jjj++;
...
int main(int argc, char** argv)
{
    ...
    path.header.frame_id = output_frame_id;
