...
namespace fsm {
    ...
    void Fsm::callReplanOnce() {
        ...
        // cout << " -- [Fsm debug] ReplanOnce started." << RESET << endl;
...
//            cout << YELLOW << " -- [Fsm] ReplanOnce failed." << RESET << endl;
    ...
    void Fsm::callMainFsmOnce() {
        ...
        switch (machine_state_) {
            ...
            case FOLLOW_TRAJ: {
                // cout << " -- [Fsm debug] FOLLOW_TRAJ started." << RESET << endl;
    ...
    void Fsm::setGoalPosiAndYaw(const Vec3f &p, const Quatf &q, const double desired_speed) {
        ...
        cout << GREEN << " -- [Fsm] Get original goal at " << RESET <<click_point.transpose() << endl;

        if (planner_ptr_->getMap()->getNearestInfCellNot(GridType::OCCUPIED, click_point, gi_.goal_p, 3.0)) {
            cout << GREEN << " -- [Fsm] Set goal at " << RESET << gi_.goal_p.transpose() << endl;
        } else {
