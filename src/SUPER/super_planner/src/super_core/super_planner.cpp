...
namespace super_planner {
    ...
    void SuperPlanner::getOneCommandFromTraj(StatePVAJ &pvaj,
                                             ...
                                             bool &on_backup_traj,
                                             bool &traj_finish) {
        // cout<<"[super debug] enter getOneCommandFromTraj"<<endl;                                       
        cmd_traj_info_.lock();
...
//        double cur_yaw = geometry_utils::get_yaw_from_quaternion(robot_state_.q);
    ...
    void SuperPlanner::getSeriesCommandFromTraj(vector<StatePVAJ> &pvajs,
                                             vector<double> &yaws,
        ...
        // cout<<"[super debug] enter getOneCommandFromTraj"<<endl;                                         
        cmd_traj_info_.lock();
        // cout<<"[super debug] before cmd_traj_info_"<<endl;
        const double &cur_t = ros_ptr_->getSimTime();
        
        const double &cmd_start_WT = cmd_traj_info_.getStartWallTime();
        // cout<<"[super debug] after cmd_traj_info_"<<endl;
        const double &total_dur = cmd_traj_info_.getTotalDuration();
        ...
        for(int i = 0 ;i< cfg_.nmpc_horizon; i++) {
            double current_eval_t = eval_t + i * cfg_.nmpc_step;
            if(current_eval_t>total_dur)
                current_eval_t = total_dur;
            StatePVAJ pvaj = cmd_traj_info_.posTraj().getState(current_eval_t);

            /// Get Yaw planning
            double yaw = cmd_traj_info_.getYaw(current_eval_t)[0];
            double yaw_dot = cmd_traj_info_.getYawRate(current_eval_t)[0];
            ...
            pvajs.push_back(pvaj);
            yaws.push_back(yaw);
            yaw_dots.push_back(yaw_dot);
        }
        cmd_traj_info_.unlock();
    }
...
//        double t_R = 0.0;
    ...
    SuperPlanner::PathSearch(const Vec3f &start_pt, const Vec3f &goal,
        ...
        if (start_type == rog_map::GridType::OCCUPIED ||
            ...
            std::cout<<YELLOW<<"start point:"<<start_pt<<std::endl;
            return false;
        }
        ...
        const double astar_timeout = 0.5;  // increased from default 0.1s for tight spaces
