...
namespace mission_planner {
    ...
    private:
        ...
        void MavrosRcCallback(const mavros_msgs::RCInConstPtr &msg) {
            ...
            int ch_10 = msg->channels[9]; // 读取遥控器第10通道
            bool pitch_up = msg->channels[1] < 1200;
            if (last_ch_10 > 1500 && ch_10 < 1500) {// 检测下降沿
                triggered = true;// 触发开始
                new_goal = true;
                waypoint_counter = 0;
    ...
    public:
        ...
        WaypointPlanner(const ros::NodeHandle &nh) {
            ...
            std::string dft_cfg_path = CONFIG_FILE_DIR("flag_waypoint_sim.yaml");
            ...
            /*LYX 2025/7/2
            此处，由于每个路点需要不同的策略，例如是否考虑避障、最大速度如何，因此单纯的geometry_msgs::PoseStamped无法满足需求。
            改用FLAG消息传递，相应的，planner内fsm处也需要相应更改
            */
            // goal_pub_ = nh_.advertise<geometry_msgs::PoseStamped>(cfg_.goal_pub_topic, 10);
            goal_pub_ = nh_.advertise<super_msgs::Flag>(cfg_.goal_pub_topic, 10);
