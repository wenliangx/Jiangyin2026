...
namespace mission_planner {
    ...
    private:
        ...
        bool CloseToPoint(Vec3f &position, int id) {
            // std::cout << "CloseToPoint? id: " << id << std::endl;
        ...
        void FlagCallback(const super_msgs::FlagConstPtr &msg){
            ...
            cout << YELLOW <<" -- [MISSION] flag triggered. waypoins size: " << cfg_.waypoints.size()<< endl;
        }

        void GoalPubTimerCallback(const ros::TimerEvent &e) {
            // std::cout<<"[MISSION DEBUG]enter pub. triggered: "<<triggered<<std::endl;
            ...
            if (CloseToPoint(cfg_.waypoints[waypoint_counter], waypoint_counter)) {//到第waypoint_counter个点的距离是否小于阈值
                ...
                if (waypoint_counter >= cfg_.waypoints.size()) {
                    // Final waypoint reached: stop entirely, do NOT republish
                    triggered = false;
                    new_goal = false;
                } else {
                    new_goal = true;
            ...
            // std::cout<<"[MISSION DEBUG]triggered."<<std::endl;

            static double last_pub_time = 0;

            if (triggered && (new_goal || cur_t - last_pub_time > cfg_.publish_dt)) {
                new_goal = false;
                ...
                super_msgs::Flag flag_state;
                flag_state.id = waypoint_counter;
                flag_state_pub_.publish(flag_state);
...
#define CONFIG_FILE_DIR(name) (string(string(ROOT_DIR) + "config/"+name))
            std::string dft_cfg_path = CONFIG_FILE_DIR("flag_waypoint.yaml");
            std::string cfg_path, cfg_name;
            if (nh.param("config_path", cfg_path, dft_cfg_path)) {
                cout << " -- [MissionPlanner] Load config from: " << cfg_path << endl;
            } else if (nh.param("config_name", cfg_name, dft_cfg_path)) {
                cfg_path = CONFIG_FILE_DIR(cfg_name);
                cout << " -- [MissionPlanner] waypoint mission Load config by file name: " << cfg_name << endl;
            }
