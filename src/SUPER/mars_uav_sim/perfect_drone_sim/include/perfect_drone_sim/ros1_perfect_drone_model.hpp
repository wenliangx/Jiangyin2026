...
namespace perfect_drone {
    ...
    public:
        PerfectDrone(const ros::NodeHandle &n) : nh_(n) {//初始化
#define CONFIG_FILE_DIR(name) (string(string(ROOT_DIR) + "config/"+name))
            ...
            cfg_ = Config(cfg_path);//从 yaml 读配置
            render_ptr_ = std::make_shared<marsim::MarsimRender>(cfg_path);//加载 3D 场景
    ...
    private:
        ...
        void cmdCallback(const super_msgs::PositionCommandConstPtr &msg) {//接受位置命令回调仿真
        ...
        void publishOdom(const ros::TimerEvent &e) {//100hz发布无人机里程计信息（不用，用rogmap）
