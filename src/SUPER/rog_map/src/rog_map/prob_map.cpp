...
void ProbMap::updateProbMap(const PointCloud& cloud, const Pose& pose) {
    ...
    if (pos.z() > cfg_.virtual_ceil_height) {
        ...
        std::cout << YELLOW <<pos.z() <<"<"<<cfg_.virtual_ground_height<<std::endl;
...
GridType ProbMap::getGridType(const Vec3f& pos) const {
    if((pos.y()*pos.y()+pos.x()*pos.x())<=1.0){
        return KNOWN_FREE;
    }
...
void ProbMap::raycastProcess(const PointCloud& input_cloud, const Vec3f& cur_odom) {
    ...
    for (const auto& pcl_p : input_cloud) {
        ...
        // 1.3) take off filter
        if (pcl_p.x*pcl_p.x + pcl_p.y*pcl_p.y + pcl_p.z*pcl_p.z < 1) {
            continue;
        }

        // 1.4) is_map filter !cfg_.is_map
        // std::cout << "is_map is " <<cfg_.is_map<< std::endl;
        if (!cfg_.is_map) {
            // std::cout << "is_map is false" << std::endl;
            resetLocalMap();
            continue;
        }
...
void ProbMap::resetLocalMap() {
    ...
    double unk_value = cfg_.l_free;//(cfg_.l_free + cfg_.l_occ)/2.0;
    // Clear local map
