// mapping_node.cpp - RA-LIO主节点：激光雷达-惯性里程计实时建图
...
inline constexpr double kInitializationTime = 0.1;
inline constexpr double kLaserPointCovariance = 0.001;
inline constexpr int kPublishFramePeriod = 20;

struct MappingNode::Impl {
  explicit Impl(ros::NodeHandle node_in) : node(std::move(node_in)) {}
  ...
  double gyr_cov = 0.1, acc_cov = 0.1, b_gyr_cov = 0.0001,
         b_acc_cov = 0.0001;  // IMU噪声协方差默认值
  double filter_size_corner_min = 0, filter_size_surf_min = 0, filter_size_map_min = 0, fov_deg = 0;
  double cube_len = 0, lidar_end_time = 0, first_lidar_time = 0.0;
  int scan_count = 0, publish_count = 0;
  int feats_down_size = 0, feats_undistort_size = 0, NUM_MAX_ITERATIONS = 0, pcd_save_interval = -1,
      pcd_index = 0;
  ...
  IncrementalMap incremental_map;

  // === LiDAR到IMU的外参 ===
  V3D Lidar_T_wrt_IMU{kZero3d};      // 平移外参
  M3D Lidar_R_wrt_IMU{kIdentity3d};  // 旋转外参
  ...
  // === 预处理和IMU处理模块 ===
  shared_ptr<Preprocessor> p_pre{std::make_shared<Preprocessor>()};
  shared_ptr<ImuProcessor> p_imu1{std::make_shared<ImuProcessor>()};
  ...
  void livox_pcl_cbk(const livox_ros_driver2::CustomMsg::ConstPtr& msg) {
    ...
    if (time_sync_en && !timediff_set_flg && abs(last_timestamp_lidar - last_timestamp_imu) > 1 &&
      ...
      timediff_lidar_wrt_imu =
          last_timestamp_lidar + 0.1 - last_timestamp_imu;  // LiDAR时间 = IMU时间 + 偏移
  ...
  void lasermap_fov_segment() {
    ...
    auto& ikdtree = incremental_map.native();
  ...
  void map_incremental() {
    ...
    for (int i = 0; i < feats_down_size; i++) {
      ...
      if (!Nearest_Points[i].empty() && flg_EKF_inited) {
        ...
        float dist =
            squaredDistance(feats_down_world->points[i], mid_point);  // 当前点到体素中心的距离
        ...
        for (std::size_t j = 0; j < kNearestNeighborCount; j++) {
          if (points_near.size() < kNearestNeighborCount) break;

          if (squaredDistance(points_near[j], mid_point) < dist)  // 已有更近的点
          {
            need_add = false;
            break;
          }
  ...
  void publish_frame_world(const ros::Publisher& pubLaserCloudFull_) {
    if (scan_pub_en) {
      ...
      publish_count -= kPublishFramePeriod;
    }
  ...
  static bool set_quaternion_msg(const Eigen::Matrix3d& rot,
  ...
  void publish_odometry(const ros::Publisher& pubOdomAftMapped) {
    ...
    br.sendTransform(
        tf::StampedTransform(transform, odomAftMapped.header.stamp, "camera_init", "body"));
    ...
    q.setZ(0);
    transform.setRotation(q);
    br_world.sendTransform(
        tf::StampedTransform(transform, odomAftMapped.header.stamp, "world", "camera_init"));
  }
  ...
  int run() {
    ...
    // === 加载配置参数 ===
    config = MappingConfig::load(nh);
    std::string validation_error;
    if (!config.valid(validation_error)) {
      ROS_FATAL_STREAM("Invalid RA-LIO configuration: " << validation_error);
      return 2;
    }
    path_en = config.path_enabled;
    speed_vector_en = config.speed_vector_enabled;
    scan_pub_en = config.scan_publish_enabled;
    dense_pub_en = config.dense_publish_enabled;
    scan_body_pub_en = config.body_frame_publish_enabled;
    NUM_MAX_ITERATIONS = config.maximum_iterations;
    map_file_path = config.map_file_path;
    lid_topic = config.lidar_topic;
    imu_topic = config.imu_topic;
    time_sync_en = config.time_sync_enabled;
    time_diff_lidar_to_imu = config.lidar_to_imu_time_offset;
    filter_size_corner_min = config.corner_leaf_size;
    filter_size_surf_min = config.surface_leaf_size;
    filter_size_map_min = config.map_leaf_size;
    cube_len = config.cube_side_length;
    DET_RANGE = config.detection_range;
    fov_deg = config.field_of_view_degrees;
    gyr_cov = config.gyro_covariance;
    acc_cov = config.acceleration_covariance;
    b_gyr_cov = config.gyro_bias_covariance;
    b_acc_cov = config.acceleration_bias_covariance;
    extrinsic_est_en = config.extrinsic_estimation_enabled;
    pcd_save_en = config.pcd_save_enabled;
    pcd_save_interval = config.pcd_save_interval;
    p_pre->configure(config.preprocessor);

    cout << "Lidar_type: " << p_pre->lidarType() << endl;
    ...
    // === 订阅和发布 ===
    // 根据雷达类型选择不同的回调函数
    ros::Subscriber sub_pcl = p_pre->lidarType() == AVIA
                                  ? nh.subscribe(lid_topic, 200000, &Impl::livox_pcl_cbk, this)
                                  : nh.subscribe(lid_topic, 200000, &Impl::standard_pcl_cbk, this);
    ros::Subscriber sub_imu = nh.subscribe(imu_topic, 200000, &Impl::imu_cbk, this);

    ros::Publisher pubLaserCloudFull =
        nh.advertise<sensor_msgs::PointCloud2>("/cloud_registered", 100000);  // 世界系去畸变点云
    ros::Publisher pubLaserCloudFull_body = nh.advertise<sensor_msgs::PointCloud2>(
        "/cloud_registered_body", 100000);  // Body(IMU)系点云
    ros::Publisher pubLaserCloudFull_lidar =
        nh.advertise<sensor_msgs::PointCloud2>("/cloud_registered_lidar", 100000);  // LiDAR系点云
    ros::Publisher pubLaserCloudEffect =
        nh.advertise<sensor_msgs::PointCloud2>("/cloud_effected", 100000);  // 有效特征点云
    ros::Publisher pubLaserCloudMap =
        nh.advertise<sensor_msgs::PointCloud2>("/Laser_map", 100000);  // 全局地图
    ros::Publisher pubOdomAftMapped =
        nh.advertise<nav_msgs::Odometry>("/Odometry", 100000);               // 里程计
    ros::Publisher pubPath = nh.advertise<nav_msgs::Path>("/path", 100000);  // 轨迹
    ros::Publisher marker_pub =
        nh.advertise<visualization_msgs::Marker>("/speed_vector", 10);  // 速度可视化

    // === 初始化外参和IMU处理参数 ===
    Lidar_T_wrt_IMU = config.lidar_to_imu_translation;
    Lidar_R_wrt_IMU = config.lidar_to_imu_rotation;

    // 设置IMU处理模块参数：外参、噪声协方差
    p_imu1->configure(ImuProcessorConfig{Lidar_T_wrt_IMU, Lidar_R_wrt_IMU, V3D::Constant(gyr_cov),
                                         V3D::Constant(acc_cov), V3D::Constant(b_gyr_cov),
                                         V3D::Constant(b_acc_cov)});
    ...
    while (ros::ok()) {
      ...
      if (sync_packages(Measures))  // 成功打包一帧LiDAR+对应的IMU数据
      {
        ...
        if (flg_first_scan) {
          ...
          p_imu1->setFirstLidarTime(first_lidar_time);
        ...
        flg_EKF_inited = (Measures.lidar_beg_time - first_lidar_time) >= kInitializationTime;
        ...
        kf.update_iterated_dyn_share_modified(kLaserPointCovariance, feats_down_body, ikdtree,
                                              Nearest_Points, NUM_MAX_ITERATIONS, extrinsic_est_en);
        ...
        std::cout << "feats_down_size: " << feats_down_size
                  << "  Whole mapping time(ms):  " << (t11 - t00) * 1000 << std::endl
                  << std::endl;
...
MappingNode::MappingNode(ros::NodeHandle node) : impl_(std::make_unique<Impl>(std::move(node))) {}
MappingNode::~MappingNode() = default;
MappingNode::MappingNode(MappingNode&&) noexcept = default;
MappingNode& MappingNode::operator=(MappingNode&&) noexcept = default;
int MappingNode::run() { return impl_->run(); }
