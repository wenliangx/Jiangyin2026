...
// 统一使用PCL标准点类型 PointXYZINormal（包含xyz、强度intensity、法向量normal_x/y/z、曲率curvature）
...
class Preprocess
{
  ...
  int  plane_judge(const PointCloudXYZI &pl, vector<orgtype> &types, uint i, uint &i_nex, Eigen::Vector3d &curr_direct); // 平面判定
  bool small_plane(const PointCloudXYZI &pl, vector<orgtype> &types, uint i_cur, uint &i_nex, Eigen::Vector3d &curr_direct);
  bool edge_jump_judge(const PointCloudXYZI &pl, vector<orgtype> &types, uint i, Surround nor_dir); // 边缘跳变判定
