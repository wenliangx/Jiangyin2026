...
**Meta:** `catkin_make` workspace root. 14 top-level entries (dirs), ~50 ROS packages total.
...
| `fsm_ctrl/` | C++14 | FSM state machine + NMPC guidance + PX4 estimator bridge (4 binaries, 6 libs, 26 GTest) |
...
| `pose_to_odom/` | — | PoseStamped → Odometry converter + fake /cloud_registered for no-LiDAR testing |
| `plane_Det/` | — | Visual plane detection. **CATKIN_IGNORE** |
| `gz_external_pose/` | Python | Gazebo model_states → VRPN pose bridge (simulation ground truth) |
| `libs/px4_plugs/` | Python | PX4 plugins: link_monitor, log_manager, param_migrator (3 Python ROS nodes) |
| `mid360_gazebo/` | — | **No package definition** — incomplete MID360 plugin. `drwx------` perms |
...
- **Python ROS nodes** in px4_plugs (3), gz_external_pose (1), uav_vision (3) — all via catkin_install_python.
