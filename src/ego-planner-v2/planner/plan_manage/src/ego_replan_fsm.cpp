...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    /*  fsm param  */
    nh.param("fsm/flight_type", target_type_, -1); // 2 FOR PRESET; 4 FOR FLAG FLY
    ...
    nh.param("manager/slow_vel", slow_speed_, 0.8);
    nh.param("manager/landing_vel", landing_speed_, 0.5);
    ...
    waypoint_num_ = 0;
    is_map_ = true;
    memset(waypoint_received_, 0, sizeof(waypoint_received_));
    memset(way_point_mode_, 0, sizeof(way_point_mode_));
    memset(waypoints_, 0, sizeof(waypoints_));
    running_mode = 0; // 1: normal mode; 2: slow mode; 0 : init mode
    ...
    ROS_INFO("CURRENT MODE IS,%d ", target_type_);

    if (target_type_ == TARGET_TYPE::MANUAL_TARGET) // pub goal
    {
      waypoint_sub_ = nh.subscribe("/goal", 1, &EGOReplanFSM::waypointCallback, this);
    }
    else if (target_type_ == TARGET_TYPE::PRESET_TARGET) // set series of waypoints
    {

      nh.param("fsm/waypoint_num", waypoint_num_, -1); // waypoint_num_  input in type 2; use in type 4 when get new waypoints(maximum))
    ...
    else if (target_type_ == TARGET_TYPE::FLAG) // FLAG FLY FSM; type = 4
    {
      flag_sub_ = nh.subscribe<traj_utils::Flag>("/ego_planner/flag_msg", 10, &EGOReplanFSM::flagCallback, this);
      flag_state_pub_ = nh.advertise<traj_utils::FlagState>("/ego_planner/flag_state", 10);
    }
    else
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    // ROS_INFO("FSM POLY SPEED, %f",planner_manager_->ploy_traj_opt_->getVelocity());
    ...
    case EXEC_TRAJ:
    {
      ...
      else if ((target_type_ == TARGET_TYPE::PRESET_TARGET || target_type_ == TARGET_TYPE::FLAG) &&
               ...
               (final_goal_ - pos).norm() < no_replan_thresh_) // case 2: assign the next waypoint
      {
        ...
        {
          wpt_id_++;
          planNextWaypoint(wps_[wpt_id_]); // global JUMP POINT}
        }
        else if (target_type_ == TARGET_TYPE::FLAG)
        {
          wpt_id_++;
          Eigen::Vector3d wp_temp(waypoints_[wpt_id_][0], waypoints_[wpt_id_][1], waypoints_[wpt_id_][2]);
          ROS_INFO("Flag FSM: Flying to ID = %d", wpt_id_);
          ROS_INFO("Flag FSM: Flying to waypoint = %f, %f, %f", wp_temp(0), wp_temp(1), wp_temp(2));
          if(way_point_mode_[wpt_id_]!=way_point_mode_[wpt_id_-1])
          {
            ROS_INFO("Flag FSM: Change Speed!");
            if(way_point_mode_[wpt_id_]==2 )
            {
              ROS_INFO("Flag FSM: CHANGE TO SLOW SPEED in FLYING");
              planner_manager_->pp_.max_vel_ = slow_speed_;
              planner_manager_->ploy_traj_opt_->setVelocity(slow_speed_);
            }
            else if(way_point_mode_[wpt_id_]==1 )
            {
              ROS_INFO("Flag FSM: CHANGE TO NORMAL SPEED in FLYING");
              planner_manager_->pp_.max_vel_ = normal_speed_;
              planner_manager_->ploy_traj_opt_->setVelocity(normal_speed_);
            }
            else if(way_point_mode_[wpt_id_]==3 )
            {
              ROS_INFO("Flag FSM: CHANGE TO LANDING SPEED in FLYING");
              planner_manager_->pp_.max_vel_ = landing_speed_;
              planner_manager_->ploy_traj_opt_->setVelocity(landing_speed_);
            }
          }
          planNextWaypoint(wp_temp);
        }
      }
      else if ((t_cur > info->duration - 1e-2) && touch_the_goal) // case 3: the final waypoint reached
      {
        ...
        else if (target_type_ == TARGET_TYPE::FLAG)
        {
          ...
          // wpt_id_ = 0;
          // planNextWaypoint(wps_[wpt_id_]);
          if (wpt_id_ == waypoint_num_ - 1)
          {
            touch_final = true;
            ROS_INFO("Flag FSM: CURRENY ID END!. ID = %d", wpt_id_);
            ROS_INFO("Flag FSM: CURRENY ID END!. waypoint_num = %d", waypoint_num_);
            running_mode = 0;
          }
        }
  ...
  void EGOReplanFSM::changeFSMExecState(FSM_EXEC_STATE new_state, string pos_call)
  {
    ...
    cout << "[" + pos_call + "]"
         << "FLAG:" << wpt_id_ << ", from " + state_str[pre_s] + " to " + state_str[int(new_state)] << endl;
  }
  ...
  {
    // cout<<next_wp(0)<<" "<<next_wp(1)<<" "<<next_wp(2)<<endl;
    bool success = false;
  ...
  void EGOReplanFSM::flagStartFirstPoint() // possible add reset speed here
  {
    // if (waypoint_num_ <= 0 || wpt_id_ <= 0)
    // {
    //   ROS_ERROR("Wrong! waypoint_num_ = %d", waypoint_num_);
    //   ROS_ERROR("Wrong! wpt_id_ = %d", wpt_id_);
    //   return;
    // }
    Eigen::Vector3d wp_temp(waypoints_[wpt_id_][0], waypoints_[wpt_id_][1], waypoints_[wpt_id_][2]);
    std::vector<Eigen::Vector3d> temp;
    temp.resize(waypoint_num_);
    for (int i = 0; i < waypoint_num_; i++)
    {
      temp[i](0) = waypoints_[i][0];
      temp[i](1) = waypoints_[i][1];
      temp[i](2) = waypoints_[i][2];
    }
    for (size_t i = 0; i < (size_t)waypoint_num_; i++)
    {
      visualization_->displayGoalPoint(temp[i], Eigen::Vector4d(0, 0.5, 0.5, 1), 0.3, i);
      ros::Duration(0.001).sleep();
    }
  ...
  void EGOReplanFSM::flagStatePub(Eigen::Vector3d pos,int id, int wp_num, int mode, int is_touch, bool is_in_obstacle)
  {
    flag_state_.header.frame_id = "world";
    flag_state_.header.stamp = ros::Time::now();
    flag_state_.header.seq = 0;
    flag_state_.now_position.x = pos(0);
    flag_state_.now_position.y = pos(1);
    flag_state_.now_position.z = pos(2);
    flag_state_.now_id=id;
    flag_state_.total_waypoint=wp_num;
    flag_state_.now_mode=mode;
    flag_state_.touch_goal=is_touch;
    flag_state_.touch_obstacle=is_in_obstacle;
    flag_state_.max_vel= planner_manager_->pp_.max_vel_;
    flag_state_pub_.publish(flag_state_);
  }
  ...
  void EGOReplanFSM::flagCallback(const traj_utils::FlagConstPtr &msg)
  {
    // have_trigger_ = true;
    cout << "Received Flag Point!" << endl;
    int id = msg->id;
    if(id>=999)
    {
        ROS_ERROR("FLAG FSM: ID %d TO BIG!!!!!!!", id);  
    } 
    int mode = msg->mode;
    if(msg->is_map==0) is_map_=false;
    else is_map_=true;
    Eigen::Vector3d wp(msg->position.x, msg->position.y, msg->position.z);
    ROS_INFO("FLAG FSM: CB TRIG! ID = %d ", id);
    ROS_INFO("FLAG FSM: CB TRIG! POS = %f, %f, %f", wp(0), wp(1), wp(2));

    way_point_mode_[id] = mode;
    ROS_INFO("FLAG FSM: CB TRIG! judge = %d ", waypoint_received_[id]);
    if (!waypoint_received_[id])
    {
      ROS_INFO("ADD an new point =%d ", id);
      ROS_INFO("Position of the new point = %f, %f, %f", wp(0), wp(1), wp(2));
      waypoint_received_[id] = 1;
      waypoint_num_ = (waypoint_num_ - 1) < id ? id + 1 : waypoint_num_; //++?
      ROS_INFO("waypoint num =%d ", waypoint_num_);
      // waypoint_num_++;
      // have_trigger_ = true;
      if (id == 0)
      {
        ...
        {
          running_mode = 2;
          ROS_INFO("Flag FSM: SLOW SPEED MODE");
          planner_manager_->pp_.max_vel_ = slow_speed_;
          planner_manager_->ploy_traj_opt_->setVelocity(slow_speed_);
        }
        else if(mode == 1)
        {
          running_mode = 1;
          ROS_INFO("Flag FSM: NORMAL SPEED MODE");
        }
        else if(mode == 3 )
        {
          // do nothing; we believe that we should not change the speed of landing in the beginning!
          running_mode = 1;
          ROS_INFO("Flag FSM: NORMAL SPEED MODE");
        }
        waypoints_[id][0] = wp(0);
        waypoints_[id][1] = wp(1);
        waypoints_[id][2] = wp(2);
        have_trigger_ = true;
        
        ROS_INFO("Flag FSM: START FIRST PLANNING!.");
        flagStartFirstPoint(); // start first point planning
        return;
      }

      if((mode == 2 && running_mode ==0)||(mode == 1 && running_mode ==0)||(mode == 3 && running_mode ==0))
      {
        if(mode == 2)
        {
          // running_mode = 2;
          ...
          planner_manager_->pp_.max_vel_ = slow_speed_;
          planner_manager_->ploy_traj_opt_->setVelocity(slow_speed_);
          ROS_INFO("CHANGE POLYSPEED, %f",planner_manager_->ploy_traj_opt_->getVelocity());
        }
        else if(mode ==1)
        {
          // running_mode = 1;
        ...
        {
          ROS_INFO("Flag FSM: LANDING SPEED MODE");
          planner_manager_->pp_.max_vel_ = landing_speed_;
          planner_manager_->ploy_traj_opt_->setVelocity(landing_speed_);  
        }
      }
    ...
    if (id < wpt_id_)
    {
      ROS_ERROR("Flag id is smaller than current wpt_id_");
      return;
    }
    else if (id == wpt_id_)
    {
      // emergency change current point
      waypoints_[id][0] = wp(0);
      waypoints_[id][1] = wp(1);
      waypoints_[id][2] = wp(2);
      if (planNextWaypoint(wp))
      {
        std::cout << "Current id " << wpt_id_ << " changed to " << wp.transpose() << std::endl;
      }
    }
    else
    {
    ...
    // FSM : STOP AND START AGAIN
    if (exec_state_ == WAIT_TARGET && id == (wpt_id_ + 1) && !running_mode)
    {
      if(mode == 2) running_mode = 2;
      else if(mode == 1) running_mode = 1;
      else if(mode == 3) running_mode = 3;

      ROS_INFO("Flag FSM: STOP AND START AGAIN!.");
      wpt_id_++;
      // changeFSMExecState(GEN_NEW_TRAJ, "FLAG");
      have_trigger_ = true;
      planNextWaypoint(wp);
    }
  }
